				...
				foreach year of local periods  {    
					...
					* For LABLAC: Load all quarters:
					if strpos("`crosscals'","lip")!=0 { // LIPI-LABLAC condition
						if "`quarters'"=="" local quarters "q01 q02 q03 q04"
						drop _all
						tempfile lipi_
						save `lipi_', replace emptyok
						foreach q of local quarters {
							noi di in white "cap  datalib, country(`country') type(lablac) period(`q') `yearsopt' `circaopt' `project' `repath' `repo' `vermast' `veralt'  incppp(ipcf ila inla ilpc) nocohh clear"
							
							cap  datalib, country(`country') type(lablac) period(`q') `yearsopt' `circaopt' ///
							`project' `repath' `repo' `vermast' `veralt'  incppp(ipcf ila inla ilpc) nocohh clear 
							if (_rc) {
					...
					* FOR SEDLAC: (non-lablac)
						...
						`project' `modules' `repath' `repo' `vermast' `veralt'  clear  
			...
			foreach country of local countries {
				...
				*************************************
				foreach p of numlist 1(2)`ny' {
					...
					noi di in white " rtwoyears is: `r(twoyears)': ny is `ny': fp is `fp' and sp is `sp'" //temp

					*Append two datasets (datalib appending isn't working and option vars isn't working.

					cap datalib, country(`country') year(`fp') `project' `modules' `vermast' `veralt' `repath' `repo' `periods' clear
					...
					cap datalib, country(`country') year(`sp') `project' `modules' `vermast' `veralt' `repath' `repo' clear 
					if (_rc) {
